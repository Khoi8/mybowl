...
| -- | ----- | ----- | ---------- | ---- | ------ |
| S1 | recognized-split lookup | BE domain | — | CI-pure | TODO |
...
- **Goal:** Pure, hardcoded recognized-split classifier keyed on the standing-pin mask, plus single-pin-leave detection, so stats never *infers* splits.
